...
I looked into the soil properties of those low leaching areas. For this, I queried the SSURGO database through the [SQL interface](http://sdmdataaccess.nrcs.usda.gov/Query.aspx) to get data on total sand and pH per component and horizon. I imported the table into the isuag PostgreSQL database. First I want to filter for the main component of each mapunit and then aggregate the variables over the horizons to get an average value for each mapunit's main component.

```{r ssurgo}
ssurgo <- tbl(isuag_db, "ssurgo_ia_soil_ph_sand_frac")
ssurgo %>%
  filter(majcompflag == "Yes") %>%
  group_by(cokey, mukey, comppct_r) %>%
  summarise(avg_sandtotal_r = mean(sandtotal_r), avg_ph1to1h2o_r = mean(ph1to1h2o_r), avg_ph01mcacl2_r = mean(ph01mcacl2_r)) %>%
  group_by(mukey) %>%
  filter(n()>1) %>%
  filter(comppct_r == max(comppct_r))
  arrange(mukey, cokey) -> ssurgo_agg

ssurgo_agg <-  collect(ssurgo_agg, n=Inf) 
...
#   group_by(mukey) %>%
#   filter(n()>1) %>%
#   filter(comppct_r == max(comppct_r)) -> multi_mukey
# head(multi_mukey)
# dim(multi_mukey)

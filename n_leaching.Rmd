...
To visualize the results I plot them in a a surface plot. I ended up using the rgl package although I am not happy with the output as a screenshot png of very low resolution, and that I cannot plot the lines between the switchgrass percent values. The alternative is persp() (commented in the below chunk), but I have not figured out how to apply color according to the z axis yet and add a legend.
```{r area_surfaceplot, fig.cap="Figure 9: Area to convert to switchgrass when assuming different thresholds for N loss and profitability of the cropland under row crops. "}


x <- profit_cutoffs
y <- n_loss_cutoffs

# create a matrix out of the x and y values for z (in Mha):
...
#        breaks = c(1,10,20,30,40,50,60,70,100), 
        border = 17, facets = TRUE, colkey = TRUE, resfac = 1, clab = "N loss reduction (%)", 
        lighting = list(alpha = .5),
        xlab = "Profit threshold", ylab = "N loss threshold", zlab = "N loss red. (%)"
        )

contour2D(z, x, y, col = "black", colkey = FALSE, add = FALSE, xlab = "Profitability threshold (US$ ha-1)", ylab = "N loss threshold (kg ha-1)")

# rect3D(x0 = 0, y0 = -1000, z0 = 20, x1 = 0, y1 = 500)

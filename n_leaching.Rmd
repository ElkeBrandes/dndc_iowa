...
I want to select the total area of polygons that fall over specific N leaching thresholds (WO scenario).
```{r swg_areas_po, cache=TRUE}
swg_areas_wo <- vector(mode="numeric", length=0)
leach_cutoffs <- c(100, 90, 80, 70, 60, 50)
 for (i in 1:length(leach_cutoffs)) {
    query <- paste("SELECT SUM(clumuha) FROM \"05_dndc_clumu_cgsb_swg\" WHERE mean_profit_ha > ", as.character(leach_cutoffs[i]))
    sum_area <- tbl(isuag_db, sql(query)) %>%
      as.data.frame()
    sum_area <- sum_area[1,1]
...
swg_area <- c(0,rep(swg_areas_po,3))
swg_area_percent <- (swg_area/9374362)*100
N_leaching <- sum_leaching[,2]
N_reduction <- abs((N_leaching - N_leaching[1])/N_leaching[1])*100

relation_po <- data.frame(swg_yield, profit_cutoff, swg_area_percent, N_reduction)

relation_wo <- data.frame(leach_cutoffs, swg_areas_wo)
```

Plot relationships:
```{r cache=TRUE, fig.show="hold", fig.cap:"Increase in NO3 leaching reduction with increasing area in switchgrass (A), increase in NO# leaching reduciton with increasing profitability threshold (B), increase of area in switchgrass with increasing profitability threshold (C)."}
ggplot() +
  geom_point(data = relation_po, aes(x = swg_area_percent, y = N_reduction, color=swg_yield)) +
  theme_b_border() +
...
ggplot() +
  geom_point(data = relation_po, aes(x = profit_cutoff, y = swg_area_percent)) +
  theme_b_border() +
  scale_x_continuous(name="Profitability threshold (US$/ha)", breaks=seq(from=-500, to=200, by= 100)) +
  scale_y_continuous(name="Area in switchgrass (%)")
```

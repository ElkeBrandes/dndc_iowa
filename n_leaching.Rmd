...
library("tmap")
library("grDevices") # for surface plot
...
To visualize the results I plot them in a a surface plot
...
x <- profit_cutoffs
#xseq <- seq(x[1], x[2], length.out = 3)
y <- n_loss_cutoffs
#yseq <- seq(y[1],y[2],length.out = 5)
...
# revers the order of the columns in the matrix (for N loss threshold):
# swg_matrix <- swg_matrix[, rev(seq_len(ncol(swg_matrix)))]
...
persp(x=x, y=y, z=swg_matrix_perc, col = color[facetcol],
      theta = -80, phi = 30, r = 5, shade = 0.50,

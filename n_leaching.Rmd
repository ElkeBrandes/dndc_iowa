...
I want to see how NO~3~^-^ leaching reduction increases with increasing area in switchgrass, to test if the disproportionate benefits hypothesis holds true in our model.

I want to select the total area of polygons that fall under specific profitability thresholds, keeping the N loss threshold constant at 60 kg ha^-1^.
...
Plot relationsships between profitability threshold and % area in switchgrass:
```{r thresholds, fig.show="hold", fig.height= 3, fig.width=4.5, fig.cap="Figure 6: Relation between the set thresholds in the profitability (left panel) and water quality optimized (right panel) scenario and the resulting area in switchgrass. N loss cut off is constant at 60 kg ha^-1^."}
ggplot() +
...
```{r plot_baseline_map, fig.cap="Figure 8: County mean annual N loss, averaged over 2012-2015, from corn and soybean fields in Iowa, are shown as a color scale. Bubbles indicate the total N that has been lost on average per year from each county."} 
...
```{r plot_n_loss_reduction_maps, fig.width=10, fig.height=3, fig.cap = "Figure 10: Reduction in total N loss in the switchgrass integration scenarios, as % of the status quo losses shown in Figure 8."}
ggplot(counties_swg_df) +

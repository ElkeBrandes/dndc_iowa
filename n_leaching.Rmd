...
I want to select the total area of polygons that fall under specific profitability thresholds.
```{r swg_areas, cache=TRUE}
swg_areas <- vector(mode="numeric", length=0)
profit <- tbl(isuag_db, "05_dndc_clumu_cgsb_swg")
cutoffs <- c(-500, -400, -300, -200, -150, -100, -50, 0, 50, 100, 150)
 for (i in 1:length(cutoffs)) {
    query <- paste("SELECT SUM(clumuha) FROM \"05_dndc_clumu_cgsb_swg\" WHERE mean_profit_ha < ", as.character(cutoffs[i]))
...
profit_cutoff <- c(NA, rep(cutoffs,3))
...
Plot the relationship between percent of area in switchgrass and percent of NO3 leaching reduction:
```{r cache=TRUE}
ggplot() +
...
Plot the relationship between profitability cut off and percent of NO3 leaching reduction:
```{r cache=TRUE}
ggplot() +
...
Plot the relationship between profitability cut off and percent area in switchgrass:

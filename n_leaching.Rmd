...
I am selecting the areas that will be managed in switchgrass based on mean profitability 2012-2015.

### State-wide mean profitability distribution

```{r profit_distribution, fig.cap="Figure 1: Distribution of mean profitability 2012-2015. The vertical line marks the -150 US$/ha cut off value used in the profit opimization (PO) scenario for switchgrass integration below."}
...
```{r histograms, cache=TRUE, fig.cap="Figure 3: Distributions of N loss in Iowa. Values were averaged over the mananagement horizon of 10 years, including two establishment years."}
...
Plot the sample:
```{r scatterplot, fig.cap="Figure 4: Scatterplot of a subsample (n=10,000) of profitability and corn/soy leaching data on subfield areas in Iowa. The red lines indicate possible cut off values, such as -150 US$ ha^-1^ for profitability (solid) and 60 kg N ha^-1^ for NO3 leaching (dashed)."}
ggplot() +
...
I looked into the soil properties of those low leaching areas. For this, I queried the SSURGO database through the [SQL interface](http://sdmdataaccess.nrcs.usda.gov/Query.aspx) to get data on total sand and pH per component and horizon. I imported the table into the isuag PostgreSQL database and joined the soil data (total sand fraction and pH), aggregated to the mapunit, to the DNDC results. 

I then draw random samples from that table to analyze the relationship between leaching and sand fraction / pH.
The theory is that high pH soils should have an equilibrium shifted toward NH~3~^+^ and therefore show higher volatilization and lower NO~3~^-^ leaching.
...
Plot the sample:
```{r scatter_leach_sand, fig.show = "hold", fig.cap="Figure 6: Scatterplot of a subsample (n=10,000) of corn/soy leaching data and sand fraction on subfield areas in Iowa."}
ggplot() +
...
```{r scatter_ph, fig.cap="Figure 7: Scatterplot of a subsample (n=10,000) of pH values measured with different methods from the SSURGO database."}
ggplot() +
...
#  labs(y=expression(pH~1~to~2~CaCl[2]),
#       x=expression(pH~1~to~1~H[2]O))
...
To eliminate artifacts that are caused by model mechanisms, we decided to sum up NO~3~^-^ leaching and NH~3~ volatilization, assuming that volatilization is negligible since it is minimized by management (i.e. timing of fertilization).

##4. N loss through NO~3~^-^ leaching and NH~3~ volatilization

I calculated N losses in PostgreSQL for corn/soybean and for the three switchgrass yield scenarios.

### Correlation between profitability and N loss in corn and soybean fields
I am using the table `05_dndc_clumu_cgsb_swg_n_loss` in the isuag database, that contains 2012-2015 mean profit and N loss values for each of the four scenarios on a subfield (clu-map unit) scale. I want to see if there is a correlation between profitability and N loss, but since the complete data set contains more than 3 million records, I want to randomly select a sub-sample for the correlation.
...
Plot the sample:
```{r scatterplot_n_loss, fig.cap="Figure 8: Scatterplot of a subsample (n=10,000) of profitability and corn/soy N loss data on subfield areas in Iowa. The red lines indicate possible cut off values, such as -150 US$ ha^-1^ for profitability (solid) and 60 kg N ha^-1^ for N loss (dashed)."}
ggplot() +
...
From figure 8 we can see that the relation between N loss and profitability is not completely random. At higher profitability, the number of sub-field patches with high N loss values decreases. The lower the profitability, the higher the heterogeneity in N loss values. In other words, by managing the areas of lowest profitability in switchgrass would target those areas of highest N loss values (but also many of lower leaching values). Managing the areas of highest N loss (e.g. above a cut off of 60 kg N/ha) would affect unprofitable as well as unprofitable areas.

We discussed in the team to target those areas that fall in the upper left quarter of the graph divided by the two red lines: Subfield areas with a profitability below -150 US$/ha and N loss above 60 kg N/ha. 

Comparison criteria:

Improvement of total N loss in form of NO~3~^-^ and NH~3~ state wide and on the county level (in % of status quo values).

N loss sums are calculated in PostgreSQL. Access the results:
```{r total_leaching}
n_loss_tot <-tbl(isuag_db, "05_dndc_n_loss_sums_iowa_scenarios")  %>%
  as.data.frame()
names(n_loss_tot) <- c("Status Quo", "7500", "10000", "12500")

n_loss_red <- data.frame(status_quo = "NA",
                        "7500" = (n_loss_tot[1,1]-n_loss_tot[1,2])*100/n_loss_tot[1,1],
                        "10000" = (n_loss_tot[1,1]-n_loss_tot[1,3])*100/n_loss_tot[1,1],
                        "12500" = (n_loss_tot[1,1]-n_loss_tot[1,4])*100/n_loss_tot[1,1])
names(n_loss_red) <- c("Status Quo", "7500", "10000", "12500")

n_loss_table <- rbind(n_loss_tot[1,],n_loss_red[1,])

kable(n_loss_table, col.names=c("Status Quo","7500", "10000", "12500"), caption="Table 1: Annual N loss (Mg N) in Iowa modelled with DNDC under different switchgrass yield scenarios." )

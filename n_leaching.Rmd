...
open3d()
rgl.surface(x=x, y=swg_matrix_perc, 
            coords=c(1,2,3),z=z, 
            color=color[ findInterval(swg_matrix_perc, seq(min(swg_matrix_perc), max(swg_matrix_perc), length=nbcol))], alpha = .5 )
axes3d()
aspect3d(1,1,1)
grid3d("x+")
grid3d("z")
grid3d("y")
#lines3d(x, swg_matrix_perc, z)
#legend3d("center", "% in switchgrass", fill= swg_matrix_perc)

par3d("windowRect")
rgl.viewpoint(zoom = .7)
rgl.snapshot("surface.png")


# nrz <- length(profit_cutoffs)
# ncz <- length(n_loss_cutoffs)
# z <- swg_matrix_perc
# 
...
# 
# persp(x=x, y=z, z=swg_matrix_perc, col = color[facetcol],
#       theta = -60, phi = 30, r = 5, shade = 0.50,
#      xlab = "Profitability cut off",
#       ylab = "N loss cut off",
#       zlab = "Area in swg (%)",
#      zlim = c(0, 100),
#       ticktype = "detailed",
...
swg_areas_11 <- sapply(area_select_profit_11, profit_cutoffs_11)

swg_area <- c(0,rep(swg_areas[33:40,3],3))

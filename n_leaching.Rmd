...
Then, read in a shape file with county delineations.
```{r}
...
# to plot spatial data in ggplot, I need to make it into a data frame to get access to the longitude and latitude:
counties_df <- fortify(counties)
head(counties_df, n = 2)
```

Join the data frame to the spatial data frame (like a join in ArcGIS):
```{r join_attributes}
cnty_leach <- merge(counties, counties_leach, by="fips")
class(cnty_leach)
head(cnty_leach@data, n=2)
```
...
head(cnty_leach_df)
ggplot(cnty_leach_df) +
  geom_path(aes(long, lat, group=group), color = "white") +
  geom_polygon(aes(long, lat, group = group, fill=ave_no3_leach_cgsb)) +
  coord_map() +
  theme_b_border()

ggplot() +
  geom_polygon(data = counties, aes(x = long, y = lat, group = group), fill=NA, color="grey") +

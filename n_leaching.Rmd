...
```
```{sql connection=con, message=FALSE}
SELECT mean_profit_ha, ave_no3_leach_ha_cgsb FROM "05_dndc_clumu_cgsb_swg" ORDER BY random() LIMIT 1
```


Run SQL queries:
```{r data_sample}
#set.seed(15) # makes r pull in the same sample every time I re-run the script
query = "SELECT mean_profit_ha, ave_no3_leach_ha_cgsb FROM \"05_dndc_clumu_cgsb_swg\" ORDER BY random() LIMIT 1"

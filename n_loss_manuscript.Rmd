...
Progress on reducing nutrient leaching from Iowa cropland has been hampered by perceived conflicts between short-term profitability and long-term stewardship. However, data suggests that perennial biomass crops such as switchgrass canmitigate NO~3~-N leaching from cropland, improve short- and long-term farm profitability, and help meet national bioenergy targets. To evaluate the potential for switchgrass to do this if integrated into Iowa's agricultural landscape, we combined results from two subfield-scale analyses: (i) a cropland profitability analysis and (ii) a mechanistic NO~3~-N leaching analysis from cropland. All data came from public sources and were analyzed using peer-reviewed, public models. For each subfield unit, we based the management change decision from row crops to switchgrass on baseline (2012-2015) profitability and NO~3~-N leaching rates, creating a matrix of possible threshold combinations. For each resulting scenario we calculated the amount of Iowa cropland in switchgrass, resulting biomass production, and relative reduction in NO~3~-N leaching compared to the baseline. We present spatially explicit leaching reduction results for a conservative scenario and a nutrient reduction scenario that respectively resulted in 12 % and 35 % of Iowa cropland converted to switchgrass, producing 11 and 33 million Mg of biomass, and reducing NO~3~-N leaching 18 % and 38 % statewide. Within the impaired Upper North Raccoon River Watershed, both scenarios showed a slightly lower reduction in NO~3~-N leaching than the statewide average. This was because while most converted subfield areas displayed large NO~3~-N leaching reductions (> 50 %), the high productivity of most cropland in this watershed kept it above the profitability threshold leading to less land converted and therefore a low overall NO~3~-N leaching reduction. Our approach bridges the scales at which NO~3~-N leaching and profitability are usually considered, and is informed by mechanistic as well as empirical understanding. Though approximated, our analysis can be turned into a farm-level decision making tool to identify locations where combined goals for farm profitability and water quality improvement can be achieved through the strategic integration of perennial vegetation.

## Highlights 
*(not needed for ERL, but can be useful anyway)*

- This is a novel use of high-resolution spatial data to test ecosystem service outcomes from precision conservation from perennial biomass crop integration

- We analyzed the impact of converting unprofitable parts of fields to biomass switchgrass on nutrient loss reduction

- We mechanistically modeled the fraction of leachable N from subfield areas of agricultural land in the state of Iowa

- Results identify where switchgrass should be targeted to meet state water quality targets while improving farm profitability


## Introduction

Agriculture is the most important industry in Iowa’s economy, generating 10% of U.S. maize exports at a value of 1.2 billion US$ in 2016 [@uscensus2016exports]. However, even highly productive cropland in Iowa has been unprofitable on average in recent years [@hart2015planting], throughing into sharp relief the often criticized practice of risking environmental quality for economic gain (REF).

Non-point nutrient pollution from industrialized agriculture in the Mississippi River Basin has been exacerbating the ~ 15,000 km^2^ hypoxic zone in the Gulf of Mexico [@turner2003linking]. Over half of the nitrogen (N) delivered through this route originate from cropland cultivated with maize (*Zea mays* L.)  and soybeans (*Glycine max* (L.) Merr.). Iowa, the second largest contributor of eutrifying nutrients to the gulf [@alexander2008differences], plays a prominent role in efforts to decrease the hypoxic zone's 5 year average to under 5000 km^2^ [@river2008action]. When this goal was missed in 2015, it was moved to 2035 by the Hypoxia Task Force [-@htf2015report]. In addition to impacting aquatic life, nitrate-N (NO~3~-N) concentrations in drinking water pose a health threat to humans. In Iowa, the North Raccoon River Watershed gained scientific and public attention when the Des Moines Waterworks sued three of its counties unter the federal Clean Water Act. This watershed provides drinking water for the state's capital and is covered 80 % with maize and soybeans [@jones2011agricultural]. The lawsuit was ultimately dismissed, but set a precedent for arguing that tile drains carrying NO~3~-N nitrate-laden water constitute a “point source” of pollution where they empty into streams.  
...
```{r figure_profit, fig.height=4, fig.width=9,  fig.cap="Figure 3: Areas below a mean 2012-2015 profitability of US$ -100 ha^-1^ per county in the baseline scenario. Purple shades indicate the relative area, orange circles show the absolute amount of land per county."}
ggplot() +
...
```{r plot_n_loss_map, cache = TRUE, fig.width=8, fig.height=4, fig.cap = "Figure 4: Mean (green shades) and total (yellow circles) annual NO~3~-N leaching per county, averaged over 2012-2015 for the baseline scenario."}
...
ggplot(counties_swg_med_df) +
  ...
  labs(fill = expression(atop("Average NO~3~-N", paste("leaching (kg N ha"^{-1}~yr^{-1},")")))) +  
...
```{r scatterplot_no3_leach, fig.width=4.5, fig.height=3, fig.cap="Figure 5: Scatterplot of a subsample (n=10,000) of subfield areas, showing profitability and NO~3~-N leaching of the baseline scenario (mean values for maize and soybean fields 2012-2015) in Iowa. The red lines and shaded areas indicate the cut off values for the conservative scenario (solid lines and deep red shade, US$ -100 ha^-1^ for profitability and 50 kg N ha^-1^ for NO~3~-N leaching) and for the nutrient reduction scenario (dashed lines and light red shade,  US$ 0 ha^-1^ for profitability and 20 kg N ha^-1^ for NO~3~-N leaching)."}
ggplot() +
...
### changes in state-wide profitability, NO~3~-N leaching, and cellulosic biomass with switchgrass integration

Baseline analysis indicated that 11.5 % of total Iowa cropland lost > US\$ 100 and leached > 50 kg N per hectare and year on average. This cropland was converted to switchgrass in the 'conservative' scenario. In total, state-wide mean annual nitrogen loss by leaching from cropland in maize/soybeans between 2012 and 2015 amounted to 0.44 million Mg. On average, converting all areas losing > US\$ 100 ha^-1^ and leaching > 50 kg N ha^-1^  to switchgrass would reduce state wide NO~3~-N leaching by 17.3, 18.1, and 18.6 % at assumed low, medium, and high yields, respectively (table 3). Because of the low sensitivity of model outcomes to switchgrass yield (table 3, figure S5), we conducted further analysis on the medium yield data set only.
...
kable(no3_table, digits = 1, format.args = list(big.mark = ','), col.names=c("Switchgrass yield/n(Mg ha^-1^)", "'Conservative'/nabs. leaching (Mg)", "'Conservative'/nrel. leaching red. (%)", "'Nutrient reduction'/nabs. leaching (Mg)", "'Conservative'/nrel. leaching red. (%)"), caption="Table 3: Average annual NO~3~-N leaching in metric tonnes (Mg) nitrogen and NO~3~-N leaching reduction relative to the baseline (%) in Iowa modelled with DNDC. Results are shown for the 'conservative' and 'nutrient reduction' scenario under three different switchgrass yield assumptions (7,500, 10,000, and 12,500 kg ha^-1^)." )

```

Moving the decision thresholds toward higher profitability and lower leaching values (i.e., targeting increasingly favorable cropland for conversion to switchgrass) resulted in successively more area in switchgrass, more switchgrass biomass production, and less NO~3~-N leached state wide (figures 6, 7). The contour plots (figure 7) serve as a tool to look up a model outcome value visualized as the "elevation" for the corresponding cut-off for profitability and NO~3~-N leaching. The conservative scenario resulted in 10.7 million Mg of switchgrass biomass and an 18.1 % reduction in NO~3~-N leaching. The 'nutrient reduction' scenario, which converted cropland of mean profitability < US\$ 0 ha^-1^ and NO~3~-N leaching > 20 kg ha^-1^, resulted in 35.0 % of cropland in switchgrass, producing 32.8 million Mg biomass, and reducing NO~3~-N leaching by 37.9 %  (dashed and solid lines in figure 7, all scenario results in tables S1-S3).
...
persp3D(x, y, z_nlr, colvar = z_nlr, phi = 10, theta = -150, col = brewer.pal(8,"YlGnBu"), 
        border = 17, facets = TRUE, colkey = TRUE, resfac = 1, clab = "NO~3~-N leaching \nreduction (%)", 
        lighting = list(alpha = 1),
...
par(xpd = NA)
text(x=-300,y=110,"(a) Cropland in switchgrass",cex = 1.5,font = 1.5)
...
par(xpd = NA)
text(x=-300,y=110,"(b) Biomass produced",cex = 1.5,font = 1.5)
...
par(xpd = NA)
text(x=-300,y=110,"(c) NO~3~-N reduction",cex = 1.5,font = 1.5)
...
Our results suggest that highest NO~3~-N leaching reduction under the 'conservative' switchgrass integration scenario can be achieved in some of the central and eastern counties (figure 8). The counties of highest NO~3~-N leaching in the baseline scenario do not necessarily line up with those of highest reduction potential. This indicates that including socio-economic thresholds concomitantly with environmental ones produces different targets for subfield intervention than would have been identified with an economic or biophysical model alone.



```{r plot_n_loss_reduction_map, cache = TRUE, fig.width=8, fig.height=4, fig.cap= "Figure 8: Relative nitrogen leaching reduction per county when switchgrass is grown on areas that loose > US$ 100 ha^-1^ and leach > 50 kg N ha^-1^, assuming medium (10,000 kg ha^-1^) switchgrass yield, as compared to the status quo in Figure 4."}
ggplot(counties_swg_med_df) +
...
The subfield resolution maps of NO~3~-N leaching reduction (figure 9) illustrate the spatial heterogeneity of environmental outcomes under the biomass crop integration scenarios. In the 'conservative' scenario, the areas in switchgrass are not evenly distributed across Iowa but clustered along the Raccoon, upper South Skunk, and Cedar Watersheds (figure 9(a)). The 'nutrient reduction' scenario would require much more cropland to be converted to switchgrass, but some regions in the north west and south east would not see significant management changes (figure 9(b)).

Focusing on the Upper North Raccoon River Watershed as an example of an impaired watershed reveals a varying pattern of switchgrass fields (figure 9(c), (d)). In some regions, whole fields are converted while in others, subfield areas are patchy and more fragmented. In the selected portion of the watershed, a region where 80 % of land is in row crops, the 'conservative' scenario (converting cropland that loses > US$ 100 and leaches > 50 kg N ha^-1^ to switchgrass) would result in 14.3 % in switchgrass and a NO~3~-N leaching reduction of 11.3 % (figure 9(c)). Most subfield areas converted to switchgrass display NO~3~-N leaching reduction of over 50 %, but the relative small conversion area (due to relatively high row crop profitability in this watershed) results in a low mean NO~3~-N leaching reduction. The 'nutrient reduction' scenario (converting all unprofitable land that is leaching > 20 kg N ha^-1^ to switchgrass) would result in 39.7 % of row crops converted to switchgrass and a NO~3~-N leaching reduction of 31.4 % (figure 9(d)).

```{r subfield_n_red_iowa_zoom, cache = TRUE, fig.height=5, fig.width=5, fig.cap="Figure 9: Subfield-scale NO~3~-N leaching reduction relative to the balseline scenario in Iowa (a, b) and in the northern part of the North Raccoon River Watershed, spanning across Buena Vista, Pocahontas, Sac, and Calhoun County (c, d). (a) and (c) show the conservative scenario where switchgrass is grown on areas that loose > US$ 100 ha^-1^ and leach > 50 kg N ha^-1^. (b) and (d) show the nutrient reduction scenario where switchgrass is grown on unprofitable areas that leach > 20 kg N ha^-1^."}

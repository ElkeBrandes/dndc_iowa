...
- Baseline: estimating the profitability and NO~3~-N leaching from  all cropland consecutively in maize and/or soybeans between 2012 and 2015
...
```{r scatterplot_no3_leach, fig.width=4.5, fig.height=3, fig.cap="Figure 5: Scatterplot of a subsample (n=10,000) of subfield areas, showing profitability and NO~3~-N leaching of the baseline scenario (mean values for maize and soybean fields 2012-2015) in Iowa. The red lines and shaded areas indicate the threshold values for the 'conservative' scenario (solid lines and deep red shade, US$ -100 ha^-1^ for profitability and 50 kg N ha^-1^ for NO~3~-N leaching) and for the 'nutrient reduction' scenario (dashed lines and light red shade,  US$ 0 ha^-1^ for profitability and 20 kg N ha^-1^ for NO~3~-N leaching)."}
ggplot() +
...
### Changes in state-wide switchgrass area, cellulosic biomass, and NO~3~-N leaching with switchgrass integration
...
kable(no3_table, digits = 1, format.args = list(big.mark = ','), col.names=c("Switchgrass yield (Mg ha^-1^)", "'Conservative' abs. leaching (Mg)", "'Conservative' rel. leaching red. (%)", "'Nutrient reduction' abs. leaching (Mg)", "'Conservative' rel. leaching red. (%)"), caption="Table 3: Average annual NO~3~-N leaching in Mg nitrogen and NO~3~-N leaching reduction relative to the baseline (%) in Iowa modelled with DNDC. Results are shown for the 'conservative' and 'nutrient reduction' scenario under three different switchgrass yield assumptions (7,500, 10,000, and 12,500 kg ha^-1^)." )
...
persp3D(x, y, z_bm, colvar = z_bm, phi = 10, theta = -150, col = brewer.pal(8,"YlOrBr"), 
        ...
        lighting = list(alpha = 1),
        xlab = "Profit threshold", ylab = "N loss threshold", zlab = "Biomass (million Mg)",
        ticktype = "detailed")
...
*Here I will insert a bar chart with N inputs and outputs for baseline, conservative, and nutrient reduction scenario. This graph is just a placeholder.*

```{r n_balance_figure, cache = TRUE, fig.height=5, fig.width=5, fig.cap="Figure 8: N balance of the baseline, 'conservative' scenario, and 'nutrient leaching' scenario." }

include_graphics("imported_images/n_balance_placeholder.PNG", dpi = 300)
```
...
```{r plot_n_loss_reduction_map, cache = TRUE, fig.width=8, fig.height=4, fig.cap= "Figure 9: Relative nitrogen leaching reduction per county when switchgrass is grown on areas that loose > US$ 100 ha^-1^ and leach > 50 kg N ha^-1^ ('conservative' scenario), assuming medium (10,000 kg ha^-1^) switchgrass yield, as compared to the status quo in Figure 4."}
ggplot(counties_swg_med_df) +
...
Our predictions for reductions in NO~3~-N leaching are consistent with field measurements and other modeling studies. As part of a high level nutrient budget for Iowa, @christianson2012nitrogen estimated NO~3~-N leaching from fertilizer inputs as 36 and 24 kg N ha^-1^ for continuous maize and maize after soybeans, respectively. Their approach did not take into account weather and soil related variability, but their results lie within our modeled distribution (figure 2 (b)). *(add a sentence regarding the state-wide N balance here.)* Total annual average NO~3~-N leaching per county (figure 4) is comparable with values for maize cropland in Iowa modeled by @davis2012impact, who found most counties in the northern half of the state losing more than 3200 Mg N. While overall modeled N balances are reasonable, 2.4 % of cropland resulted in very low leaching values that coincided with ammonia volatilization rates above the state wide average (figure 2 (b)). These areas were concentrated in the three north-western counties Lyon, Plymouth, and Woodbury (figure S2). High volatilization occurs at a combination of warm temperatures, sufficient soil moisture and high soil pH. *(add explanation - hopefully high pH values in those soils)*
...
*in the following, I want to point out the strengths and the limitations of the study, and relate them to the novelty, i.e. the combination of high spatial resolution and extent. I haven't figured out how this can be best achieved yet...*

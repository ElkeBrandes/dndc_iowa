---
title: Addressing farm economy, water quality, and bioenergy goals by sub-field integration of switchgrass on unprofitable cropland
author: "Elke Brandes"
...
- We modelled the leachable N fraction for agriculatural lands in the state of Iowa
...
1. Quantify the extent of DIN (dissolved inorganic nitrogen) loss reduction by managing cropland areas that are at highest economic and environmental risk in switchgrass
...
z <- swg_matrix_perc
p1 <- persp3D(x, y, z, colvar = z, phi = 10, theta = -150, col = brewer.pal(8,"YlGn"), 
...
z_bm <- swg_matrix_bm
p2 <- persp3D(x, y, z_bm, colvar = z_bm, phi = 10, theta = -150, col = brewer.pal(8,"YlGn"), 
...
p3 <- persp3D(x, y, z_nlr, colvar = z_nlr, phi = 10, theta = -150, col = brewer.pal(8,"YlGn"), 
...
```{r area_contourplot, fig.show="hold", fig.height=8, fig.width=8, fig.cap="Figure 3: Surface plot (vertical projection of Figure 2) showing the resulting areas converted to switchgrass (in % of row cropland) with varying combinations of N loss and profitability threshold values. Lines with numbers 10-70 show the threshold combinations with similar areas converted to switchgrass. The grey dashed lines show an example how to read the plot: If cropland losing > 60 kg N ha^-1^ and losing > US$ 150 ha^-1^ was considered for conversion to switchgrass, approx. 10% of cropland currently in rowcrop would be in switchgrass."}

# create data frame for facet plots


mylevels <- c(seq(0,70,10),100)
filled.contour2(x, y, z, col = brewer.pal(length(mylevels),"YlGn"), levels=mylevels, xlab=expression(Profitability~threshold~("US$"~ha^{-1})), ylab=expression(N~loss~threshold~(kg~ha^{-1})), xlim = c(-400,700), ylim = c(0,120))
contour(x, y, z, levels=mylevels, add = TRUE, xlim = c(-400,700), ylim = c(0,120))
abline(h = 60, col = "dark gray", lty = 2)
abline(v = -150, col = "dark gray", lty = 2)

filled.contour2(x, y, z_bm, col = brewer.pal(length(mylevels),"YlGn"), levels=mylevels, xlab=expression(Profitability~threshold~("US$"~ha^{-1})), ylab=expression(N~loss~threshold~(kg~ha^{-1})), xlim = c(-400,700), ylim = c(0,120))
contour(x, y, z_bm, levels=mylevels, add = TRUE, xlim = c(-400,700), ylim = c(0,120))
abline(h = 60, col = "dark gray", lty = 2)
abline(v = -150, col = "dark gray", lty = 2)

# mylevels2 <- c(seq(0,60,10),100)
filled.contour2(x, y, z_nlr, col = brewer.pal(length(mylevels),"YlGn"), levels=mylevels, xlab=expression(Profitability~threshold~("US$"~ha^{-1})), ylab=expression(N~loss~threshold~(kg~ha^{-1})), xlim = c(-400,700), ylim = c(0,120))
contour(x, y, z_nlr, levels=mylevels, add = TRUE, xlim = c(-400,700), ylim = c(0,120))
abline(h = 60, col = "dark gray", lty = 2)
abline(v = -150, col = "dark gray", lty = 2)

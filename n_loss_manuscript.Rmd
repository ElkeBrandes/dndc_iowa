...
Using publicly available data, we combined a subfield profitability analysis with the Landscape Environmental Assessment Framework (LEAF) developed by @muth2013integrated to assess environmental impacts of land management (Fig. 1).
...
Mean profitability in 2012-2015 ranged from US\$ -478 ha^-1^ to US\$ 385 ha^-1^ on 95 % of cropland (disregarding the lowest and highest 2.5 %, respectively) with a median of US$ 53 ha^-1^. Twenty-three percent of cropland was losing > US\$ 100 ha^-1^ (Fig. 2a). Aggregating these areas of highly unprofitable cropland to the county level revealed spatial heterogeneity in its relative and absolute extent across Iowa. The counties with the most highly unprofitable land were in west and central Iowa and those with the highest relative areas were in the west, central, and south part of the state (Fig. 3). NO~3~-N leaching ranged from 9 to 113 kg N ha^-1^, when disregarding the 2.5 % of lowest and highest leaching cropland, respectively, with a median of 42 kg N ha^-1^ (Fig. 2b). The spatial pattern of NO~3~-N leaching in Iowa shows highest leaching in the counties on the Des Moines Lobe Landform (Fig. S6) and in the north east of the state (Fig. 4). The relationship between mean profitability and mean NO~3~-N leaching is characterized by an increased variability in leaching with decreasing profitability (Fig. 5).
...
theme_gcbb <- function (base_size = 10, base_family = "") 
{
  theme_grey(base_size = base_size, base_family = base_family) %+replace% 
    theme(
...
Moving the decision thresholds toward higher profitability and lower leaching values (i.e., targeting increasingly favorable cropland for conversion to switchgrass) resulted in successively more area in switchgrass, more switchgrass biomass production, and less NO~3~-N leached state wide (Fig. 6, 7). The contour plots (Fig. 7) serve as a tool to look up a model outcome value visualized as the "elevation" for the corresponding threshold for profitability and NO~3~-N leaching. The 'conservative' scenario resulted in 10.7 million Mg of switchgrass biomass and an 18.1 % reduction in NO~3~-N leaching. The 'nutrient reduction' scenario, which converted cropland of mean profitability < US\$ 0 ha^-1^ and NO~3~-N leaching > 20 kg ha^-1^, resulted in 35.0 % of cropland in switchgrass, producing 32.8 million Mg biomass, and reducing NO~3~-N leaching by 37.9 %  (dashed and solid lines in Fig. 7, all scenario results in tables S1-S3). We selected this scenario even though it did not result in the 41 % reduction required in the Nutrient Reduction Strategy, because to reach this goal, profitable cropland would have to be converted to switchgrass (Fig. 7).
...
, nrow=16, ncol=16)
#calculate % of total cropland (9413900 ha)
tot_area <- 9.413900
swg_matrix_perc <- (swg_matrix/tot_area)*100
z <- swg_matrix_perc


tiff('fig7.tif', width = 169, height = 300, units = "mm", res = 300)
...
# top left plot (swg area surface):
par(new = "TRUE",
    pin = c(3,3), 
    plt = c(0.05,0.5,0.65,0.9),   # using plt instead of mfcol (compare
...
# top right plot (swg area contour):
par(new = "TRUE",
    pin = c(3,3), 
    plt = c(0.65,0.95,0.7,0.9),   # using plt instead of mfcol (compare
                                  # coordinates in other plots)
    las = 1,                      # orientation of axis labels
    cex.axis = 1,                 # size of axis annotation
    tck = -0.02 )                  # major tick size and direction, < 0 means outside
...
#Middle left plot (biomass surface):

par(new = "TRUE",
    pin = c(3,3),
    plt = c(0.15,0.5,0.35,0.6),  # defining window for second plot
    las = 1,
    cex.axis = 1)
...
# Middle right plot (biomass contour):

par(new = "TRUE",
    pin = c(3,3),
    plt = c(0.6,0.8,0.4,0.6),  # defining window for second plot
    las = 1,
    cex.axis = 1)
...
#Bottom left plot (nut. red. surface):

par(new = "TRUE",
    pin = c(3,3),
    plt = c(0.15,0.5,0.05,0.3),
    las = 1,
    cex.axis = 1)
...
# Bottom right plot (nut. red. contour):

par(new = "TRUE",
    pin = c(3,3),
    plt = c(0.6,0.8,0.1,0.3),
    las = 1,
    cex.axis = 1)
...
Our results suggest that highest NO~3~-N leaching reduction under the 'conservative' and 'nutrient reduction' switchgrass integration scenario can be achieved in some of the central, eastern, and southern counties (Fig. 9). The counties of highest NO~3~-N leaching in the baseline scenario do not necessarily line up with those of highest reduction potential. 
...
The subfield resolution maps of NO~3~-N leaching reduction (Fig. 10) illustrate the spatial heterogeneity of environmental outcomes under the biomass crop integration scenarios. In the 'conservative' scenario, the areas in switchgrass are not evenly distributed across Iowa but clustered along the Raccoon, upper South Skunk, and Cedar Watersheds (Fig. 10a). The 'nutrient reduction' scenario would require much more cropland to be converted to switchgrass, but some regions in the north west and south east would not see significant management changes (Fig. 10b).

Focusing on the Upper North Raccoon River Watershed as an example of an impaired watershed reveals a varying pattern of switchgrass fields (Fig. 10c,d). In some regions, whole fields are converted while in others, subfield areas are patchy and more fragmented. In the selected portion of the watershed, a region where 80 % of land is in row crops, the 'conservative' scenario (converting cropland that loses > US$ 100 and leaches > 50 kg N ha^-1^ to switchgrass) would result in 14.3 % in switchgrass and a NO~3~-N leaching reduction of 11.3 % (Fig. 10c). Most subfield areas converted to switchgrass display NO~3~-N leaching reduction of over 50 %, but the relative small conversion area (due to relatively high row crop profitability in this watershed) results in a low mean NO~3~-N leaching reduction. The 'nutrient reduction' scenario (converting all unprofitable land that is leaching > 20 kg N ha^-1^ to switchgrass) would result in 39.7 % of row crops converted to switchgrass and a NO~3~-N leaching reduction of 31.4 % (Fig. 10d).

...
Mean profitability in 2012-2015 ranged from US\$ -478 ha^-1^ to US\$ 385 ha^-1^ on 95 % of cropland (disregarding the lowest and highest 2.5 %, respectively) with a median of US$ 53 ha^-1^. Twenty-three percent of cropland was losing > US\$ 100 ha^-1^ (Fig. 2a). Aggregating these areas of highly unprofitable cropland to the county level revealed spatial heterogeneity in its relative and absolute extent across Iowa. The counties with the most highly unprofitable land were in west and central Iowa and those with the highest relative areas were in the west, central, and south part of the state (Fig. 3). NO~3~-N leaching ranged from 13 to 114 kg N ha^-1^, when disregarding the 2.5 % of lowest and highest leaching cropland, respectively, with a median of 41 kg N ha^-1^ (Fig. 2b). The spatial pattern of NO~3~-N leaching in Iowa shows highest leaching in the counties on the Des Moines Lobe Landform (Fig. S7) and in the north east of the state (Fig. 4). The relationship between mean profitability and mean NO~3~-N leaching is characterized by an increased variability in leaching with decreasing profitability (Fig. 5).
...
plot_profit <- ggplot(profit, aes(profit_ha_rounded, weight = area_Mha)) + 
  ...
  scale_y_continuous() +
  annotate("text", x = -960, y = .4, label = "(a)", size = 5) +
...
Baseline analysis indicated that 12 % of total Iowa cropland lost > US\$ 100 and leached > 50 kg N per hectare and year on average. This cropland was converted to switchgrass in the 'conservative' scenario. In total, state-wide mean annual N loss by leaching from cropland in maize/soybeans between 2012 and 2015 amounted to 0.45 million Mg. On average, converting all areas losing > US\$ 100 ha^-1^ and leaching > 50 kg N ha^-1^  to switchgrass would reduce state wide NO~3~-N leaching by 17.6, 18.4, and 18.8 % at assumed low, medium, and high switchgrass yields, respectively (table 3). Because of the low sensitivity of model outcomes to switchgrass yield (Table 3, Fig. S6), we conducted further analysis on the medium yield data set only.
...
Moving the decision thresholds toward higher profitability and lower leaching values (i.e., targeting increasingly favorable cropland for conversion to switchgrass) resulted in successively more area in switchgrass, more switchgrass biomass production, and less NO~3~-N leached state wide (Fig. 6, S4). The contour plots (Fig. S4) serve as a tool to look up a model outcome value visualized as the "elevation" for the corresponding threshold for profitability and NO~3~-N leaching. The 'conservative' scenario resulted in 11.3 million Mg of switchgrass biomass and an 18.4 % reduction in NO~3~-N leaching. The 'nutrient reduction' scenario, which converted cropland of mean profitability < US\$ 0 ha^-1^ and NO~3~-N leaching > 20 kg ha^-1^, resulted in 36.6 % of cropland in switchgrass, producing 34.3 million Mg biomass, and reducing NO~3~-N leaching by 38.1 %  (dashed and solid lines in Fig. S4, all scenario results in tables S1-S3). We selected this scenario even though it did not result in the 41 % reduction required in the Nutrient Reduction Strategy, because to reach this goal, profitable cropland would have to be converted to switchgrass (Fig. S4).
...
At the state-wide level, the difference in N input sources between the baseline and the 'conservative' switchgrass integration scenario is mainly attributed a decreased fertilization rate (Fig. 7). Amongst the output N components, largest decreases are apparent in NO~3~-N leaching and runoff. In both scenarios, output exceeded the input N, leading to an overall decrease in soil N. Net N mineralization has not been included in this balance since this process was not adequately represented by the model.

```{r n_balance_figure, fig.height=5, fig.width=5, fig.cap="Figure 7: Nitrogen balance of the baseline, 'conservative', and 'nutrient reduction' scenario. Values are averaged over the modeled time period (4 years for row crops and 10 years for switchgrass). Numbers above each set of N inputs and outputs indicate the overall N balance." }
...
Our results suggest that highest NO~3~-N leaching reduction under the 'conservative' and 'nutrient reduction' switchgrass integration scenario can be achieved in some of the central, eastern, and southern counties (Fig. 8). The counties of highest NO~3~-N leaching in the baseline scenario do not necessarily line up with those of highest reduction potential. 



```{r plot_n_loss_reduction_map, fig.width=8, fig.height=4, fig.cap= "Figure 8: Relative reduction in nitrogen leaching per county when switchgrass is grown on areas that loose > US$ 100 ha^-1^ and leach > 50 kg N ha^-1^ ('conservative' scenario), assuming medium (10,000 kg ha^-1^) switchgrass yield, as compared to the status quo in Figure 4."}
...
The subfield resolution maps of NO~3~-N leaching reduction (Fig. 9) illustrate the spatial heterogeneity of environmental outcomes under the biomass crop integration scenarios. In the 'conservative' scenario, the areas in switchgrass are not evenly distributed across Iowa but clustered along the Raccoon, upper South Skunk, and Cedar Watersheds (Fig. 9a). The 'nutrient reduction' scenario would require much more cropland to be converted to switchgrass, but some regions in the north west and south east would not see significant management changes (Fig. 9b).

Focusing on the Upper North Raccoon River Watershed as an example of an impaired watershed reveals a varying pattern of switchgrass fields (Fig. 9c,d). In some regions, whole fields are converted while in others, subfield areas are patchy and more fragmented. In the selected portion of the watershed, a region where 80 % of land is in row crops, the 'conservative' scenario (converting cropland that loses > US$ 100 and leaches > 50 kg N ha^-1^ to switchgrass) would result in 14.3 % in switchgrass and a NO~3~-N leaching reduction of 11.3 % (Fig. 9c). Most subfield areas converted to switchgrass display NO~3~-N leaching reduction of over 50 %, but the relative small conversion area (due to relatively high row crop profitability in this watershed) results in a low mean NO~3~-N leaching reduction. The 'nutrient reduction' scenario (converting all unprofitable land that is leaching > 20 kg N ha^-1^ to switchgrass) would result in 39.7 % of row crops converted to switchgrass and a NO~3~-N leaching reduction of 31.4 % (Fig. 9d).

```{r subfield_n_red_iowa_zoom, cache = TRUE, fig.height=5, fig.width=5, fig.cap="Figure 9: Subfield-scale NO~3~-N leaching reduction relative to the baseline scenario in Iowa (a, b) and in the northern part of the North Raccoon River Watershed, spanning across Buena Vista, Pocahontas, Sac, and Calhoun County (c, d). (a) and (c) show the 'conservative' scenario where switchgrass is grown on areas that loose > US$ 100 ha^-1^ and leach > 50 kg N ha^-1^. (b) and (d) show the 'nutrient reduction' scenario where switchgrass is grown on unprofitable areas that leach > 20 kg N ha^-1^."}
...
The authors thank Marshall McDaniels for his valuable expert input in discussions about soil nitrogen processes.
